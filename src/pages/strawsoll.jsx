...
function PollOption({ id, label, votes, total, voted, vote }) {
  return (
    <button
      ...
      className="bg-white items-center flex relative w-[300px] h-20 text-lg p-4 rounded-xl outline-none border-2 border-blue-500"
      style={{ cursor: voted ? 'default' : 'pointer'}}
    >
      {
        voted && (
          <div 
            className="h-full absolute rounded-[9px] top-0 left-0 bg-blue-300"
            style={{
                width: `${(votes / total * 300) - 4}px`
            }}
          />
        )
      }
  ...
  return (
    <div className="flex flex-col items-center justify-center gap-4 w-full h-full">
      <div className="mb-10 flex flex-col gap-3">
        <h1 className="text-5xl text-gray-900 font-bold text-center">Your time to vote!</h1>
        {
          voted && (
